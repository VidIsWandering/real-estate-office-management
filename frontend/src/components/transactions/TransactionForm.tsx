...
export function TransactionForm() {
  ...
  const [transactionStatus, setTransactionStatus] = useState<string>("negotiating");
    ...
    <div className="flex flex-col gap-6 p-1 max-h-[80vh] overflow-y-auto custom-scrollbar">
      ...
      <div className="flex flex-col gap-8">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-muted-foreground">Property (BM3)</label>
              <Select onValueChange={setPropertyId}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a property..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="BDS001">BDS001 - Townhouse (District 1)</SelectItem>
                  <SelectItem value="BDS002">BDS002 - Apartment (District 7)</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <div className="space-y-1.5">
              <label className="text-sm font-medium text-muted-foreground">Customer (BM2)</label>
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-muted-foreground">Initial agreement notes</label>
            <Textarea 
        ...
        <section className="space-y-4">
          <h3 className="text-lg font-semibold border-b pb-2">Transaction status</h3>
          ...
          <div className="flex flex-wrap gap-4 p-4 bg-slate-50 rounded-lg border">
            {[
              ...
              <div key={status.id} className="flex items-center gap-2 min-w-[140px]">
                <Checkbox
                  id={status.id}
                  checked={transactionStatus === status.id}
                  onCheckedChange={(checked) => checked && setTransactionStatus(status.id)}
                />
                <label htmlFor={status.id} className="text-sm font-medium cursor-pointer">
                  {status.label}
                </label>
              </div>
            ))}
          ...
          {transactionStatus === "cancelled" && (
            <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1">
              ...
              <Textarea placeholder="Why was this cancelled?" className="min-h-[80px]" />
            </div>
          )}
        </section>
      </div>
