    ...
    <div className="border border-gray-200 rounded-xl h-24 p-1 relative bg-white">
      {!empty && (
        <div className="flex flex-col gap-1">
          {events.includes("P") && (
            <EventChip label="P" color="bg-blue-100 text-blue-700" />
          )}
          {events.includes("H") && (
            <EventChip label="H" color="bg-yellow-100 text-yellow-700" />
          )}
          {events.includes("Cl") && (
            <EventChip label="Cl" color="bg-purple-100 text-purple-700" />
          )}
          {events.includes("Mark") && (
            <EventChip label="Mark" color="bg-green-100 text-green-700" />
          )}
        </div>
      )}
    </div>
...
function EventChip({ label, color }: { label: string; color: string }) {
  return (
    <div
      className={`px-2 py-0.5 text-xs rounded-md font-medium w-fit ${color}`}
    >
      {label}
    </div>
  );
