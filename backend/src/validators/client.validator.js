...
const createValidator = [
  ...
  body('type')
    ...
    .withMessage(
      `Type must be one of: ${Object.values(CLIENT_TYPES).join(', ')}`
    ),
...
const getAllValidator = [
  ...
  query('full_name')
    .optional()
    .isString()
    .withMessage('Full name filter must be a string'),
  ...
  query('phone_number')
    .optional()
    .isMobilePhone('vi-VN')
    .withMessage('Phone number filter must be a valid Vietnamese phone number'),

  query('type')
    .optional()
    .isIn(Object.values(CLIENT_TYPES))
    .withMessage(
      `Type filter must be one of: ${Object.values(CLIENT_TYPES).join(', ')}`
    ),

  query('address')
    .optional()
    .isString()
    .withMessage('Address filter must be a string'),

  query('staff_id')
    .optional()
    .isInt({ gt: 0 })
    .withMessage('Staff ID must be a positive integer'), // 🔹 Thêm staff_id
];
