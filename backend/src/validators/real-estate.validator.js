...
const { TRANSACTION_TYPES, DIRECTIONS, REAL_ESTATE_STATUS } = require("../config/constants");
...
const createRealEstateValidator = [
  ...
  body("transaction_type")
    ...
    .withMessage(`Transaction type must be one of: ${Object.values(TRANSACTION_TYPES).join(", ")}`),
  ...
  body("direction")
    ...
    .withMessage(`Direction must be one of: ${Object.values(DIRECTIONS).join(", ")}`),
...
const getAllRealEstateValidator = [
  ...
  query("status")
    ...
    .withMessage(`Status must be one of: ${Object.values(REAL_ESTATE_STATUS).join(", ")}`),
];
