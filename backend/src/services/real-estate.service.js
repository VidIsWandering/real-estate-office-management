...
class RealEstateService {
  ...
  async updateRealEstateById(realEstateId, updateData) {
    ...
    const existingRealEstate = await realEstateRepository.findByLocation(updateData.location)
    const isIncluded = existingRealEstate.find(item => {
      console.log(item.id)
      return item.id = realEstateId
    }) ? true : false;
    console.log(isIncluded)
    if (existingRealEstate.length > 0 && !isIncluded) throw new Error("A real estate property already exists at this location")
    const media_files = await fileService.createManyFiles(updateData.media_files)
    const legal_docs = await fileService.createManyFiles(updateData.legal_docs)

    const mediaFileIds = media_files.items.map(item => item.id)
    const legalDocIds = legal_docs.items.map(item => item.id)

    await realEstateRepository.update(realEstateId, { ...updateData, media_files: mediaFileIds.length > 0 ? mediaFileIds : null, legal_docs: legalDocIds.length > 0 ? legalDocIds : null })
