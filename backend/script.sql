...
-- 2. STAFF
-- ============================================================================
CREATE TYPE staff_role_enum AS ENUM ('admin', 'manager', 'agent', 'staff');
...
CREATE TYPE real_estate_status_enum AS ENUM ('created', 'pending', 'listed', 'negotiating', 'transacted', 'suspended');
...
-- 8. TRANSACTION
-- ============================================================================
CREATE TYPE transaction_status_enum AS ENUM ('negotiating', 'pending', 'cancelled');
...
CREATE TYPE contract_status_enum AS ENUM ('draft', 'pending', 'signed', 'notarized', 'finalized', 'cancelled');
...
INSERT INTO account (username, password) VALUES 
    ('admin', '$2a$10$dummyhash'),
    ('manager1', '$2a$10$dummyhash'),
    ('agent1', '$2a$10$dummyhash');

-- Sample staff
INSERT INTO staff (account_id, full_name, email, role, status) VALUES 
...
-- Comments
-- ============================================================================
COMMENT ON TABLE account IS 'Tài khoản đăng nhập';
COMMENT ON TABLE staff IS 'Nhân viên (có role: admin, manager, agent, staff)';
COMMENT ON TABLE client IS 'Khách hàng';
COMMENT ON TABLE real_estate IS 'Bất động sản';
COMMENT ON TABLE appointment IS 'Lịch hẹn xem nhà';
COMMENT ON TABLE transaction IS 'Giao dịch';
COMMENT ON TABLE contract IS 'Hợp đồng';
COMMENT ON TABLE voucher IS 'Chứng từ thu chi';

COMMENT ON TYPE staff_role_enum IS 'Vai trò nhân viên: admin (quản trị), manager (quản lý), agent (môi giới), staff (nhân viên)';
...
COMMENT ON TYPE transaction_type_enum IS 'Loại giao dịch: sale (bán), rent (cho thuê)';
